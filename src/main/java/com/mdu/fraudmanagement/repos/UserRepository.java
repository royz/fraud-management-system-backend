...
//	public String findUserIdAvailability(String userId);
    ...
    @Query(value = "SELECT * from users u WHERE u.user_id=?1 AND u.ans1=?2 AND u.ans2=?3 AND u.ans3=?4 ",
            nativeQuery = true)
    public User findByUserIdAndAns(String userId, String ans1, String ans2, String ans3);

    @Query(value = "SELECT * from users u WHERE u.contact_no=?1 AND u.ans1=?2 AND u.ans2=?3 AND u.ans3=?4 ",
            nativeQuery = true)
